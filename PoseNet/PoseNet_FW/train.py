...
def main(epochs, batch_size, learning_rate, save_freq, data_dir):
    ...
    # load dict
    p = os.path.join('checkpoints', 'epoch_latest.pth')
    if os.path.exists(p):
      posenet.load_state_dict(torch.load(p))
    ...
    for epoch in range(epochs):
        ...
        if epoch % save_freq == 0:
            ...
            save_filename = 'epoch_latest.pth'
...
def get_args():
    parser = OptionParser()
    parser.add_option('--epochs', default=50, type='int')
    parser.add_option('--learning_rate', default=0.00005)
    parser.add_option('--batch_size', default=75, type='int')
    parser.add_option('--data_dir', default='data/datasets/KingsCollege/')
    parser.add_option('--save_freq', default=48)
